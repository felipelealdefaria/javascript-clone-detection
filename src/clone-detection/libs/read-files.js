...
const isDirectory = (dirPath, file) =>
...
const addTreeWithNameHashed = (tree) => {
  ast.walk(tree, (node) => {
    if (node?.type === 'FunctionDeclaration' && node?.id?.name)
      node.id.name = `${node.id.name}_${Math.floor(Math.random() * 999 + 1)}`
  })
  ast.walk(tree, (node, parent) => {
    if (node?.type === 'ArrowFunctionExpression' && parent?.id?.name)
      parent.id.name = `${parent.id.name}_${Math.floor(
        Math.random() * 999 + 1
      )}`
  })
}
...
getAllFiles(process.argv[2]).map((file) => {
  ...
  fileToTree.body.forEach((tree) => {
    ...
    if (tree?.declaration?.type === 'FunctionDeclaration')
      arrayFiles.push(tree.declaration)

    if (
      tree?.declarations &&
      tree?.declarations[0]?.init?.type === 'ArrowFunctionExpression'
    )
      arrayFiles.push(tree)
    if (
      tree?.declaration?.declarations &&
      tree?.declaration?.declarations[0]?.init?.type ===
...
console.log(`\n⚡ QUANTIDADE DE ARQUIVOS ANALISADOS: ${countFiles} ⚡\n`)

arrayFiles.forEach((tree) => {
  const contentBase = fs.readFileSync(
    `${process.cwd()}/__base-functions.js`,
    'utf-8'
  )
...
parseFile.body.map((el) => {
  ...
  if (
    el?.declarations &&
    el?.declarations[0]?.init?.type === 'ArrowFunctionExpression'
  )
    arrayNames.push(el?.declarations[0]?.id?.name)
...
console.log(
  `\n⚡⚡ ${
    arrayFunctions.length
  } FUNÇÕES EXTRAÍDAS FORAM GERADAS EM: ${process.cwd()}/__base-functions.js ⚡⚡`
)

cloneDetection(
  arrayNames,
  arrayFunctions.map((el) => ast.generate(el)),
  process.argv[3]
)
